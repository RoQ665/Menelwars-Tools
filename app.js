...
(() => {
  ...
  function experimentalUiAllowed(account=cachedAccountStatus) {
    return Boolean(
      account &&
      account.admin &&
  ...
  function experimentalUiWarmAttentionData() {
    ...
    experimentalUiWarmInFlight=Promise.allSettled([
      fetchApprovedRecipes({force:false}),
      gardenFetchData({force:false}),
      loadPayments({background:true}),
      loadGangDemandGlobal({force:false})
    ]).finally(()=>{
      experimentalUiWarmAt=Date.now();
      experimentalUiWarmInFlight=null;
      experimentalUiRefreshAttention();
    });
    return experimentalUiWarmInFlight;
  }

  function experimentalUiMark(selector,level="",label="") {
    document.querySelectorAll(selector).forEach(button=>{
      if (level) {
        button.dataset.attention=level;
        button.dataset.attentionLabel=label;
        button.title=label;
      } else {
        if (button.title===button.dataset.attentionLabel) button.removeAttribute("title");
        delete button.dataset.attention;
        delete button.dataset.attentionLabel;
      }
    });
  }

  function experimentalUiBuildIncomplete() {
    try {
      const profile=buildProfileStats(buildState);
      const hasAnyData=Boolean(
        buildPointsUsed()>0 ||
        profile.bonusesConfirmed ||
          ...
          ? inProgress.map(recipe => {
              ...
              return `
                <article
                  ...
                  ${reservationNeedsAttention?'<span class="experimental-card-attention" aria-label="Rezerwacja kończy się w ciągu godziny">!</span>':""}
                  <div>
  ...
  function paymentsRow(player,index=0) {
    ...
    return `
      <div class="finance-player-row ${stateClass} ranked-payment-row">
        ${ownPaymentProblem?'<span class="experimental-card-attention experimental-card-attention-critical" aria-label="Dług lub brak możliwości kopania">!</span>':""}
...
function renderGangDemandGlobal(payload) {
  ...
  box.innerHTML=groups.size?[...groups.entries()].map(([itemId,requests])=>{
    ...
    return `<section class="gang-demand-group"><div class="gang-demand-group-head">${gangDemandIconGlobal(item)}<div class="gang-demand-group-head-main"><strong>${escapeHtml(item.name)}</strong><small>${requests.length} ${requests.length===1?"osoba potrzebuje":"osoby potrzebują"} · łącznie ${total} szt.${!requests.every(entry=>entry.completed)?` · zadeklarowano ${Math.min(total,offered)}`:""}</small></div></div><div class="gang-demand-requests">${requests.map(entry=>{
      ...
      const closeButton=entry.canClose&&!entry.completed?`<button type="button" class="gang-demand-close" data-gang-demand-close="${escapeHtml(entry.id)}">${entry.isOwner?"✅ Potwierdź odbiór":entry.canDelete?"🗑 Usuń":"✅ Załatwione"}</button>`:"";
...
async function gangDemandCloseGlobal(id,isDelete) {
  ...
  try {
    ...
    if (status) status.textContent=isDelete
      ? "✅ Wpis usunięty z aktywnego zapotrzebowania."
      : "✅ Odbiór potwierdzony. Zapotrzebowanie zostało zakończone.";
    gangDemandCacheGlobal=null;
...
function specialOpsRenderStashGlobal(payload){
  ...
  const stackTotal=(payload.stash||[]).reduce((sum,stash)=>sum+(items.get(stash.itemKey)?.kind==="stack"?Number(stash.quantity)||0:0),0);
  const stackCap=stackTotal?Math.max(1,Math.ceil(stackTotal*0.1)):0;
  const rules=stackTotal?`<div class="submit-info special-ops-rules"><b>Wielosztuki: ${stackTotal} szt. · limit ${stackCap} na gracza</b><span>Losowanie rotacyjne według kolejności poniżej. Gdy liczba pozostałych sztuk zrówna się z liczbą uprawnionych osób bez nagrody, system przydzieli końcówkę po jednej sztuce, o ile pozwalają na to preferencje.</span></div>`:"";
  const cards=(payload.stash||[]).map(stash=>{
  ...
  box.innerHTML=rules+cards;
}

function specialOpsRenderResultsGlobal(payload){
  const box=el("special-ops-results"),batch=payload.batch;if(!box)return;
  const auditRows=(payload.audit||[]).filter(row=>row.action==="manual_close"||row.action==="departed_prizes_cancelled"||(row.action==="import_history"&&Number(row.details?.units)>0)||(row.action==="preference"&&row.actorKey!==row.targetNickKey)).slice(0,20);
  const auditHtml=auditRows.length?`<details class="special-ops-proof"><summary>📜 Historia zmian (${auditRows.length})</summary>${auditRows.map(row=>{
    ...
    if(row.action==="departed_prizes_cancelled")return `<article><b>${escapeHtml(row.details?.nick||target)} opuścił gang — anulowano nieprzekazane przydziały</b><small>${specialOpsDateGlobal(row.createdAt)} · ${Math.max(0,Number(row.details?.cancelledPrizes)||0)} szt. pozostało w schowku do kolejnego losowania${row.details?.autoClosed?" · serię zamknięto automatycznie":""}</small></article>`;
  ...
  for(const draw of batch.draws.filter(draw=>draw.status!=="cancelled")){
  ...
  box.innerHTML=`<div class="special-ops-batch-head"><b>${status}</b><small>Losował: ${escapeHtml(batch.createdBy)} · ${specialOpsDateGlobal(batch.createdAt)}</small>${batch.closeReason?`<p>${escapeHtml(batch.closeReason)}</p>`:""}</div><div class="special-ops-result-columns"><section><h4>Według przedmiotów</h4>${[...byItem.values()].map(item=>`<article><strong>${escapeHtml(item.name)}</strong>${[...item.rows].map(([nick,count])=>`<span>${escapeHtml(nick)} <b>×${count}</b></span>`).join("")}</article>`).join("")}</section><section><h4>Według graczy</h4>${[...byPlayer].map(([nick,items])=>`<article><strong>${escapeHtml(nick)}</strong>${[...items].map(([name,count])=>`<span>${escapeHtml(name)} <b>×${count}</b></span>`).join("")}</article>`).join("")}</section></div><details class="special-ops-proof"><summary>🔎 Publiczny zapis losowania (${batch.draws.length})</summary>${batch.draws.map((draw,index)=>{const guaranteed=(draw.candidates||[]).some(row=>row.finalGuarantee),cancelled=draw.status==="cancelled";return `<article><b>${index+1}. ${escapeHtml(draw.itemName)} → ${escapeHtml(draw.winner)}${cancelled?" · ANULOWANO":""}</b><small>${cancelled?`${escapeHtml(draw.note||"Gracz opuścił gang; przedmiot pozostał w schowku.")} · `:""}${guaranteed?"Końcowe zabezpieczenie · ":""}Liczba losowa: ${(Number(draw.randomRatio)*100).toFixed(6)}% · kandydatów: ${draw.candidates.length}</small><details><summary>Wagi i przedziały</summary><ul>${draw.candidates.map(row=>`<li><span>${escapeHtml(row.nick)} · waga ${Number(row.weight).toLocaleString("pl-PL")}</span><b>${(Number(row.chance)*100).toFixed(4)}% · ${(Number(row.rangeStart)*100).toFixed(4)}–${(Number(row.rangeEnd)*100).toFixed(4)}%</b></li>`).join("")}</ul></details></article>`;}).join("")}</details>${auditHtml}`;
}
...
function setupAdmin() {
  ...
  function pvpDamageFormula(attacker,defender,round,params,isCrit,isFirst,options={}) {
    ...
    // K zależy od poziomu BRONIĄCEGO, nie atakującego. Pełny log xBuLax
    // kontra RoQ (obaj lvl 55) kalibruje zwykłe buildy graczy na całej tabeli:
    // 485 ATK, 21,75% przebicia, 1126 DEF i 53% DR daje 73 dmg do T12,
    // a następnie 74 / 76 / 77. Odtwarza to K ~= 256–257, czyli
    // 175 + 1,5 × (poziom - 1). Dokładne bossy mają własny defenseK.
  ...
  function pvpApplyBleedTick(victim,round) {
    ...
    // Pełny log RoQ kontra Mateusz Rowerzysta potwierdził, że odporność jest
...
fetchModuleAccessPolicy().catch(()=>{});
  ...
  el("experimental-ui-enabled")?.addEventListener("change",event=>{
    if (!experimentalUiAllowed()) {
