...
function renderGangDemandGlobal(payload) {
  ...
  const entries=all.filter(entry=>gangDemandFilterGlobal==="completed"?entry.completed:gangDemandFilterGlobal==="mine"?!entry.completed&&(entry.isOwner||(entry.offers||[]).some(offer=>offer.isMine&&!offer.withdrawn&&!offer.expired)):!entry.completed);
  const groups=new Map();
  ...
  box.innerHTML=groups.size?[...groups.entries()].map(([itemId,requests])=>{
    ...
    const offered=requests.reduce((sum,entry)=>sum+(entry.offers||[]).filter(offer=>!offer.withdrawn&&!offer.expired).reduce((part,offer)=>part+Number(offer.amount||0),0),0);
    return `<section class="gang-demand-group"><div class="gang-demand-group-head">${gangDemandIconGlobal(item)}<div class="gang-demand-group-head-main"><strong>${escapeHtml(item.name)}</strong><small>${requests.length} ${requests.length===1?"osoba potrzebuje":"osoby potrzebują"} · łącznie ${total} szt.${!requests.every(entry=>entry.completed)?` · zadeklarowano ${Math.min(total,offered)}`:""}</small></div></div><div class="gang-demand-requests">${requests.map(entry=>{
      const amount=Math.max(1,Number(entry.amount)||1),offers=Array.isArray(entry.offers)?entry.offers:[],activeOffers=offers.filter(offer=>!offer.withdrawn&&!offer.expired),given=activeOffers.reduce((sum,offer)=>sum+Number(offer.amount||0),0),percent=Math.min(100,Math.round(given/amount*100));
      ...
      const offerChips=offers.length?`<div class="gang-demand-offers">${offers.map(offer=>`<span class="gang-demand-offer-chip${offer.withdrawn||offer.expired?" withdrawn":""}"${offer.expiresAt?` title="${offer.expired?"Wygasła":"Wygasa"}: ${escapeHtml(gangAnnouncementDate(offer.expiresAt))}"`:""}>${escapeHtml(offer.nick)}: ${Number(offer.amount)||0}${offer.expired?" · wygasła":""}${offer.canWithdraw&&!offer.withdrawn&&!offer.expired?` <button type="button" data-demand-offer-withdraw="${escapeHtml(offer.id)}" title="Wycofaj deklarację">×</button>`:""}</span>`).join("")}</div>`:"";
