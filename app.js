...
function specialOpsRenderResultsGlobal(payload){
  ...
  box.innerHTML=`<div class="special-ops-batch-head"><b>${status}</b><small>Losował: ${escapeHtml(batch.createdBy)} · ${specialOpsDateGlobal(batch.createdAt)}</small>${batch.closeReason?`<p>${escapeHtml(batch.closeReason)}</p>`:""}</div><div class="special-ops-result-columns"><section><h4>Według przedmiotów</h4>${[...byItem.values()].map(item=>`<article><strong>${escapeHtml(item.name)}</strong>${[...item.rows].map(([nick,count])=>`<span>${escapeHtml(nick)} <b>×${count}</b></span>`).join("")}</article>`).join("")}</section><section><h4>Według graczy</h4>${[...byPlayer].map(([nick,items])=>`<article><strong>${escapeHtml(nick)}</strong>${[...items].map(([name,count])=>`<span>${escapeHtml(name)} <b>×${count}</b></span>`).join("")}</article>`).join("")}</section></div><details class="special-ops-proof"><summary>🔎 Publiczny zapis losowania (${batch.draws.length})</summary>${batch.draws.map((draw,index)=>{const guaranteed=(draw.candidates||[]).some(row=>row.finalGuarantee);return `<article><b>${index+1}. ${escapeHtml(draw.itemName)} → ${escapeHtml(draw.winner)}</b><small>${guaranteed?"Końcowe zabezpieczenie · ":""}Liczba losowa: ${(Number(draw.randomRatio)*100).toFixed(6)}% · kandydatów: ${draw.candidates.length}</small><details><summary>Wagi i przedziały</summary><ul>${draw.candidates.map(row=>`<li><span>${escapeHtml(row.nick)} · waga ${Number(row.weight).toLocaleString("pl-PL")}</span><b>${(Number(row.chance)*100).toFixed(4)}% · ${(Number(row.rangeStart)*100).toFixed(4)}–${(Number(row.rangeEnd)*100).toFixed(4)}%</b></li>`).join("")}</ul></details></article>`;}).join("")}</details>${auditHtml}`;
}
